...
| `--clipboard-notify-on-save <BOOL>` | `CLIPBOARD_NOTIFY_ON_SAVE` | `true` | 自动保存成功后发送系统通知（macOS） |
...
export CLIPBOARD_NOTIFY_ON_SAVE=true
...
- **系统通知**: 自动保存成功后（`CLIPBOARD_NOTIFY_ON_SAVE=true`）会发送系统通知（当前为 macOS）。
